...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!loginLiberado($email)) {
        $erro = 'Muitas tentativas. Aguarde ' . segundosRestantesBloqueio($email) . ' segundos e tente novamente.';
    } elseif ($tipo === 'cliente') {
        if (loginCliente($email, $senha, $pdo)) { limparFalhasLogin($email); header('Location: dashboard_cliente.php'); exit; }
        else { registrarFalhaLogin($email); $erro = 'E-mail ou senha inválidos.'; }
    } else {
        if (loginEmpresa($email, $senha, $pdo)) { limparFalhasLogin($email); header('Location: dashboard_empresa.php'); exit; }
        else { registrarFalhaLogin($email); $erro = 'E-mail ou senha inválidos.'; }
    }
}
?>
...
<div class="auth-split">
  ...
  <aside class="auth-panel">
    <a href="index.php" class="auth-panel-back">&larr; Voltar ao início</a>
